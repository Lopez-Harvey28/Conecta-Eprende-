...
export function useCurrentUser(){
  ...
  return{authenticated:true as const,id:account.id,name:account.displayName,email:account.email,providerId:provider?.id??null,clientProfileId:clientProfile?.id??null,phoneVerified:!!account.phoneVerifiedAt,isAdmin:session.systemRoles.includes("ADMIN"),role:session.systemRoles.includes("ADMIN")?"ADMIN" as const:provider?"PROVIDER" as const:"CLIENT" as const,systemRoles:session.systemRoles};
}
