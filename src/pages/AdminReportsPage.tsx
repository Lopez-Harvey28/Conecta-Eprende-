...
export default function AdminReportsPage(){
  const current=useMvpStore(state=>state.currentUser);const reports=useMvpStore(state=>state.reports);const providers=useMvpStore(state=>state.providers);const update=useMvpStore(state=>state.updateReport);const [filter,setFilter]=useState<Report["status"]|"">("PENDING");
  if(current.role!=="ADMIN")return <div className="content-page"><EmptyState icon={<ShieldAlert/>} title="No autorizado">Iniciá una sesión administrativa para revisar reportes.</EmptyState></div>;
